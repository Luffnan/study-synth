  ...
  return new Promise((resolve, reject) => {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      try {
        ...
        const contentBlocks = [];
        const rawFileBuffers = []; // for source file storage after note is saved
        ...
        for (const file of uploaded) {
          ...
          rawFileBuffers.push({ fileName: file.originalFilename || file.newFilename, buffer: rawBuffer, mimeType: mime, fileSize: rawBuffer.length });
        }

        resolve({ contentBlocks, fileNames, yearLevel, rawFileBuffers });
      } catch (e) { reject(e); }
    });
  });
}
