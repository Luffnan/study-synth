...
// Lightweight update — only patches the notes JSON column (e.g. after diagram URL injection).
// Does NOT touch file_names, concise_notes, or metadata columns.
export async function updateNotesJson(id, notes, userId) {
  const query = supabase.from('notes').update({ notes }).eq('id', id);
  if (userId) query.eq('user_id', userId);
  const { error } = await query;
  if (error) throw new Error(error.message);
}
